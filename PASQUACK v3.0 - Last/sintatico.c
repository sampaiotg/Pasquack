...
int main() {
    ...
    if(vaiLer==1) le_num_assembly();
    dotLit(procasm,gerFile);
    fputs("end main\n", gerFile);
    fclose(gerFile);
    printTable();
    char str[1000];
    ...
    if(match(PRINT)) {
        ...
        if(match(LPARENT)) {
            if(ident()) {
                ...
                emit("\tpush cx\n");
                emit("\tcall Le_prox\n");
                sprintf(instr,"\tmov [%s],cx\n",id);
                emit(instr);
                emit("\tpop cx\n");
                emit("\tpop bx\n");
                emit("\tpop ax\n");
                while(match(VIRGULA)) {
                    strcpy(id,lookahead->lexeme);
                    emit("\tpush ax\n");
                    emit("\tpush bx\n");
                    emit("\tpush cx\n");
                    emit("\tcall Le_prox\n");
        ...
        if(match(PONTOVIRGULA)) {
            int label_number;
            sprintf(instr,"\tjmp EndLabel%d",label_loop);
            emit(instr);
            emit(" ;break\n");
            return 1;
        } else return 0;
...
void emit_proc(char instruction[100]) {
    //printf(instruction);
    strcat(procasm,instruction);
}
void le_num_assembly() {
    emit_proc("Le_prox proc near\n");
    emit_proc("Inicio:\n");
    emit_proc("\tmov ah,01h\n");
    emit_proc("\tint 21h\n");
    emit_proc("\tcmp al,0dh\n");
    emit_proc("\tje fim_proc\n");
    emit_proc("\tjmp Verifica\n");
    emit_proc("Numero:\n");
    emit_proc("\tpush ax\n");
    emit_proc("\tmov bx,00ah\n");
    emit_proc("\tmov ax,cx\n");
    emit_proc("\tmul bx ;ax  = ax*bx\n");
    emit_proc("\tmov bx,ax\n");
    emit_proc("\tpop ax\n");
    emit_proc("\tadd bx,ax\n");
    emit_proc("\tmov cx,bx\n");
    emit_proc("\tjmp Inicio\n");
    emit_proc("Verifica:\n");
    emit_proc("\tsub al,30h\n");
    emit_proc("\tcmp al,9\n");
    emit_proc("\tja fim\n");
    emit_proc("\tcmp al,0\n");
    emit_proc("\tjb fim\n");
    emit_proc("\tjmp Numero\n");
    emit_proc("fim_proc:\n");
    emit_proc("\tret\n");
    emit_proc("Le_prox endp\n");

    emit_proc("int2ascii proc near\n");
    emit_proc("\tmov bx, 0aH ; coloca 10 em bx\n");
    emit_proc("INICIO_CONVERSAO:\n");
    emit_proc("\tdiv bx ; divide ax por bl --> quociente em al, resto em ah\n");
    emit_proc("\tcall converte_e_armazena\n");
    emit_proc("\tcmp al,0\n");
    emit_proc("\tje FIM_CONVERSAO\n");
    emit_proc("\txor ah,ah\n");
    emit_proc("\tjmp INICIO_CONVERSAO\n");
    emit_proc("FIM_CONVERSAO:\n");
    emit_proc("\tret\n");
    emit_proc("int2ascii endp\n");

    emit_proc("converte_e_armazena proc near\n");
    emit_proc("\tpush bx\n");
    emit_proc("\tpush ax\n");
    emit_proc("\tpush cx\n");
    emit_proc("\txor ch,ch\n");
    emit_proc("\tmov cl, [Tam]\n");
    emit_proc("\tmov si, cx\n");
    emit_proc("\tmov bx, OFFSET Vetor\n");
    emit_proc("\tadd ah, 30h\n");
    emit_proc("\tmov [bx][si],ah\n");
    emit_proc("\tdec cx\n");
    emit_proc("\tmov [Tam],cl\n");
    emit_proc("\tpop cx\n");
    emit_proc("\tpop ax\n");
    emit_proc("\tpop bx\n");
    emit_proc("\tret\n");
    emit_proc("converte_e_armazena ENDP\n");
