...
void ini_symbol_table() {
    ...
    initializeSymbolTable("byte", BYTE, -1);
    initializeSymbolTable("word", WORD, -1);
    initializeSymbolTable("single", SINGLE, -1);
    initializeSymbolTable("double", DOUBLE, -1);
    initializeSymbolTable("extended", EXTENDED, -1);
    initializeSymbolTable("comp", COMP, -1);
}
    ...
    if(isalpha(look1)) {
        ...
        *buffer = '\0';
        ...
        if(token2 != NULL) {
            ...
            if((token2->tag > 287 && token2->tag < 295) || (token2->tag > 345 && token2->tag < 352) || (token2->tag == END) || (token2->tag==BREAK) || token2->tag == READKEY) {
                if (token2->tag == END || token2->tag == BREAK || token2->tag == READKEY) {
                    //fseek(file, -(sizeof(char)), SEEK_CUR);
                    //look1 = fgetc(file);
                    //printf("tok2=%s\n",token2->lexeme);
                   // printf("look1=%c\n",look1);
                    if(look1 = ';' && look1!='.') {
                       // printf("; deoius di end\n");
                        fseek(file, -(sizeof(char)), SEEK_CUR);
                        //return token2;
                    } //else {
                       // look1 = fgetc(file);
...
void printTable(void) {
    ...
    while(pos < current->numTokens) {
        if(current->token[pos]->tag == LITERAL)
            fprintf(file,"%10s %15i %60s |%3i\n",current->token[pos]->lexeme,current->token[pos]->tag, current->token[pos]->literal, current->token[pos]->value);
        pos ++;
    }
    fclose(file);
    //system(saidalex);
...
int geralit(char *fileN) {
    FILE *leitura;
    char saidalex[20];
    strcpy(saidalex,fileName);
    strcat(saidalex,".lex");
    leitura = fopen(saidalex,"r");
    char saida[1001],aux[85];
    int c,d,n=0,x=0;
    while(fgets(saida,1000,leitura)!= NULL) {
        char str[80];
        if((saida[0] != '#') && (saida[1] != '!')) {
            c=26;
            d=0;
            while(c<99) {
                 if((saida[c] == ' ') && (saida[c+1] == ' ')) {
                     do {
                         c++;
                     } while(saida[c]==' ');
                 } else {
